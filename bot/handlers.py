...
from bot.deepseek_client import extract_goals_from_conversation, generate_evening_analysis, analyze_dream
...
async def send_morning_message(bot_instance, telegram_id: int, suggestions: str):
    """Отправляет утренние рекомендации и предложение записать сон."""
    await bot_instance.send_message(telegram_id, suggestions)
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if goals:
        ...
        welcome = (
            ...
            "• Каждое утро предлагаю 3-5 маленьких, комфортных шагов к ним\n"
            "• Ты ведёшь дневник — записываешь любые события, мысли, наблюдения в дневник\n"
            "• Вечером мы вместе подводим итог дня: я даю тёплый разбор и вопросы для рефлексии\n"
            "• В любой момент можно изменить или дополнить цели, желания и записи через меню\n\n"
...
async def start_goal_session(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "Я помогу выделить главное и сформулировать цели и желания.\n\n"
    ...
    if current_state == GoalSession.active:
        ...
        if temp_goals:
            ...
            await bot.send_message(telegram_id,
                f"⏰ Время опроса вышло. Мы успели наметить такие цели и желания:\n\n{goals_text}\n\n"
...
async def confirm_goals(callback: CallbackQuery, state: FSMContext):
    ...
    temp_goals = data.get("temp_goals", [])
    if not temp_goals:
        await callback.message.edit_text("Нет целей/желаний для сохранения.")
        await callback.answer()
        return
...
async def delete_goal_list(callback: CallbackQuery):
    ...
    if not goals_with_ids:
        await callback.message.edit_text(
            "У тебя нет целей/желаний для удаления.",
            reply_markup=goals_menu_keyboard()
        )
        await callback.answer()
        return
...
async def resetdb_command(message: Message):
    ...
    await message.answer(
        ...
        "Будут удалены все пользователи, цели, записи дневника, утренние советы, вечерние итоги и сны.\n"
        "Это действие необратимо.\n\nВы уверены?",
        parse_mode="Markdown",
...
async def confirm_resetdb(callback: CallbackQuery):
    ...
    async with db.pool.acquire() as conn:
        await conn.execute("TRUNCATE TABLE users, goals, journal_entries, evening_summaries, morning_suggestions, dream_analyses CASCADE")
        ...
        await conn.execute("ALTER SEQUENCE dream_analyses_id_seq RESTART WITH 1")
    await callback.message.edit_text(
        "✅ База данных полностью очищена.\nВсе пользователи, цели, записи и сны удалены.\nБот готов к новому старту."
    )
    await callback.answer()
